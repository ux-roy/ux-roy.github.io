document.addEventListener('DOMContentLoaded', () => {
    ...
    if (customCursor && window.innerWidth > 768) {
        ...
        const updateCursor = () => {
            // Easing effect - lowered to 0.1 for a more "natural" / fluid feel
            cursorX += (mouseX - cursorX) * 0.1;
            cursorY += (mouseY - cursorY) * 0.1;
            
            // Centralized offset for 16px: -8px
            // If hovering, combine with rotation
            const rotation = customCursor.dataset.hovering ? 'rotate(45deg)' : 'rotate(0deg)';
            customCursor.style.transform = `translate3d(${cursorX - 8}px, ${cursorY - 8}px, 0) ${rotation}`;
        ...
        const interactiveElements = document.querySelectorAll('button, a, .menu-item, .panel-link');
        interactiveElements.forEach(el => {
            el.addEventListener('mouseenter', () => {
                customCursor.dataset.hovering = 'true';
                
                // Color logic based on current theme
                const isDarkMode = document.documentElement.getAttribute('data-theme') === 'dark';
                if (isDarkMode) {
                    customCursor.style.color = '#ffffff'; // White in Dark Mode
                } else {
                    customCursor.style.color = '#000000'; // Black in Light Mode
                }
            });
            el.addEventListener('mouseleave', () => {
                delete customCursor.dataset.hovering;
                // Always back to Orange default
                customCursor.style.color = 'var(--accent-color)';
            });
        });
    }
});
