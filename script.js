document.addEventListener('DOMContentLoaded', () => {
    ...
    if (customCursor) {
        ...
        const hoverables = document.querySelectorAll('a, button, .panel-link, [role="button"]');
        ...
        // Handle dynamic elements by delegated event if needed
        document.addEventListener('mouseover', (e) => {
            if (e.target.closest('a, button, .panel-link, [role="button"]')) {
                customCursor.classList.add('active');
            } else {
                // If we're using a single mouseover listener, we need to handle leaving.
                // But the above mouseenter/mouseleave is safer for most elements.
            }
        });
    }
});
